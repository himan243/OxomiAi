...
import { Lock, ShieldCheck, CheckCircle2, Trash2, Filter, Search, RefreshCcw, Eye, MapPin, X, ArrowRight, Edit3, Save } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#fdfcf7] pt-24 md:pt-32 pb-20 px-4 md:px-6 lg:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-stone-100 mb-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <div className="relative">
              ...
              <input type="text" placeholder="Search titles..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-stone-50 rounded-2xl border-none font-bold text-stone-900 focus:ring-4 focus:ring-amber-100" />
            </div>
        ...
        <div className="flex gap-4 mb-8">
          <button onClick={() => setActiveTab('content')} className={`px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all ${activeTab === 'content' ? 'bg-amber-800 text-white shadow-lg scale-105' : 'bg-white text-stone-400 hover:text-amber-800'}`}>Stories & Media</button>
          <button onClick={() => setActiveTab('suggestions')} className={`px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all relative ${activeTab === 'suggestions' ? 'bg-amber-800 text-white shadow-lg scale-105' : 'bg-white text-stone-400 hover:text-amber-800'}`}>
            Edit Suggestions
            {suggestions.length > 0 && <span className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-[10px] border-2 border-[#fdfcf7]">{suggestions.length}</span>}
          </button>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-stone-50">
              <thead className="bg-stone-50/50">
                <tr>
                  <th className="px-6 md:px-10 py-6 text-left text-[10px] font-black text-stone-400 uppercase tracking-widest">{activeTab === 'content' ? 'Story' : 'Suggested Improvement'}</th>
                  <th className="hidden md:table-cell px-10 py-6 text-left text-[10px] font-black text-stone-400 uppercase tracking-widest">{activeTab === 'content' ? 'Location' : 'Original Story'}</th>
                ...
                <AnimatePresence mode="wait">
                  {activeTab === 'content' ? (
                    filteredStories.length > 0 ? filteredStories.map((story) => (
                      <motion.tr key={story.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => { setSelectedStory(story); setIsDirectEditing(false); }} className="hover:bg-stone-50/50 transition-colors group cursor-pointer">
                        <td className="px-6 md:px-10 py-6 md:py-8">
                          <div className="flex items-center gap-4 md:gap-6">
                            <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl overflow-hidden bg-stone-100 shadow-sm flex-shrink-0">
                              {story.type === 'video' ? <div className="w-full h-full flex items-center justify-center bg-stone-200"><Eye size={20} className="text-stone-400" /></div> : <img src={story.media_url} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" alt="" />}
                            </div>
                            <div>
                        ...
                        <td className="hidden md:table-cell px-10 py-8"><div className="flex items-center gap-2 text-stone-600 font-bold capitalize text-sm"><MapPin size={14} className="text-amber-600" />{story.district}</div></td>
                        <td className="px-6 md:px-10 py-8"><div className={`inline-flex px-3 md:px-4 py-1.5 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest ${story.status === 'approved' ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'}`}>{story.status}</div></td>
                        <td className="px-6 md:px-10 py-8 text-right"><ArrowRight size={20} className="inline text-stone-200 group-hover:text-amber-600 group-hover:translate-x-2 transition-all" /></td>
                      </motion.tr>
                    )) : <tr><td colSpan={4} className="px-10 py-32 text-center"><div className="w-20 h-20 bg-stone-50 rounded-full flex items-center justify-center mx-auto mb-6 text-stone-200"><Filter size={32} /></div><p className="text-stone-400 font-black uppercase tracking-widest text-xs">No stories found</p></td></tr>
                  ) : (
                    suggestions.length > 0 ? suggestions.map((sugg) => (
                      <motion.tr key={sugg.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedSuggestion(sugg)} className="hover:bg-stone-50/50 transition-colors group cursor-pointer">
                        <td className="px-6 md:px-10 py-6 md:py-8"><div><p className="font-black text-stone-900 md:text-lg tracking-tight mb-1">{sugg.suggested_title}</p><p className="text-stone-400 text-[8px] md:text-[10px] font-black uppercase tracking-widest">New Proposal</p></div></td>
                        <td className="hidden md:table-cell px-10 py-8"><div className="text-stone-600 font-bold text-sm">{sugg.cultural_content?.title || 'Unknown Story'}</div></td>
                        <td className="px-6 md:px-10 py-8"><div className="inline-flex px-3 md:px-4 py-1.5 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest bg-blue-50 text-blue-600">{sugg.status}</div></td>
                        <td className="px-6 md:px-10 py-8 text-right"><ArrowRight size={20} className="inline text-stone-200 group-hover:text-amber-600 group-hover:translate-x-2 transition-all" /></td>
                      </motion.tr>
                    )) : <tr><td colSpan={4} className="px-10 py-32 text-center"><div className="w-20 h-20 bg-stone-50 rounded-full flex items-center justify-center mx-auto mb-6 text-stone-200"><RefreshCcw size={32} /></div><p className="text-stone-400 font-black uppercase tracking-widest text-xs">No pending suggestions</p></td></tr>
                  )}
                </AnimatePresence>
              </tbody>
            </table>
          ...
          <div className="fixed inset-0 z-[5000] flex items-center justify-center p-2 md:p-10 overflow-hidden">
            ...
            <motion.div className="relative bg-white w-full max-w-6xl h-full md:h-[85vh] rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row z-10">
              <div className="w-full md:w-3/5 h-[45vh] md:h-auto bg-stone-900 relative">
                {selectedStory.type === 'video' ? <video src={selectedStory.media_url} controls autoPlay className="w-full h-full object-contain" /> : <img src={selectedStory.media_url} alt={selectedStory.title} className="w-full h-full object-cover" />}
              ...
              <div className="w-full md:w-2/5 p-8 md:p-16 overflow-y-auto bg-[#fdfcf7] flex flex-col">
                <div className="flex justify-between items-start mb-10">
                  <div className="inline-flex flex-col gap-2">
                    <div className="inline-flex items-center gap-2 bg-amber-50 text-amber-800 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border border-amber-100">{selectedStory.category}</div>
                    <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] w-fit ${selectedStory.status === 'approved' ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'}`}>Current: {selectedStory.status}</div>
                  </div>
                ...
                {isDirectEditing ? (
                  <div className="space-y-6 flex-1">
                    ...
                    <div>
                      ...
                      <textarea value={editDesc} onChange={(e) => setEditDesc(e.target.value)} className="w-full bg-white rounded-xl px-5 py-4 border-2 border-amber-100 focus:border-amber-500 outline-none font-medium text-lg h-64 resize-none leading-relaxed" />
                    </div>
                  </div>
                ) : (
                  <>
                    <h3 className="text-4xl md:text-5xl font-black text-stone-900 mb-8 tracking-tighter leading-[0.9]">{selectedStory.title}</h3>
                    <div className="flex flex-wrap items-center gap-6 mb-10 text-stone-400">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-amber-200 flex items-center justify-center text-amber-800 font-black text-xs">{selectedStory.contributor?.[0]}</div>
                        <div><p className="text-[10px] font-black uppercase tracking-widest text-stone-300 mb-0.5">Contributor</p><p className="text-sm font-bold text-stone-900 uppercase tracking-tight">{selectedStory.contributor}</p></div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-stone-100 flex items-center justify-center text-stone-400"><MapPin size={18} /></div>
                        <div><p className="text-[10px] font-black uppercase tracking-widest text-stone-300 mb-0.5">District</p><p className="text-sm font-bold text-stone-900 uppercase tracking-tight capitalize">{selectedStory.district}</p></div>
                      </div>
                    </div>
                    <div className="flex-1"><p className="text-stone-600 text-lg font-medium leading-relaxed whitespace-pre-wrap">{selectedStory.description}</p></div>
                  </>
                )}
                ...
                <div className="mt-12 pt-10 border-t border-stone-100 flex flex-col gap-4">
                  {isDirectEditing ? (
                    <div className="flex gap-4">
                      <button onClick={handleDirectUpdate} className="flex-1 flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] py-5 rounded-2xl bg-green-600 text-white hover:bg-green-700 transition shadow-lg"><Save size={18} /> Save Changes</button>
                      <button onClick={() => setIsDirectEditing(false)} className="px-8 py-5 rounded-2xl bg-stone-100 text-stone-600 font-black text-xs uppercase tracking-[0.2em] hover:bg-stone-200">Cancel</button>
                    </div>
                  ) : (
                    <>
                      <div className="flex gap-4">
                        <button onClick={() => handleToggleStatus(selectedStory.id, selectedStory.status)} className={`flex-1 flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] py-5 rounded-2xl transition-all shadow-lg ${selectedStory.status === 'approved' ? 'bg-amber-100 text-amber-800 hover:bg-amber-200' : 'bg-green-600 text-white hover:bg-green-700'}`}>{selectedStory.status === 'approved' ? <RefreshCcw size={18} /> : <CheckCircle2 size={18} />}{selectedStory.status === 'approved' ? 'Move to Pending' : 'Approve Story'}</button>
                        <button onClick={() => handleDelete(selectedStory.id)} className="p-5 bg-red-50 text-red-600 rounded-2xl hover:bg-red-100 transition shadow-sm border border-red-100"><Trash2 size={24} /></button>
                      </div>
                      <button onClick={startDirectEdit} className="w-full flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] py-5 rounded-2xl bg-amber-800 text-white hover:bg-amber-900 transition shadow-lg"><Edit3 size={18} /> Direct Edit</button>
                    </>
                  )}
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-[5000] flex items-center justify-center p-2 md:p-10 overflow-hidden">
            ...
            <motion.div className="relative bg-white w-full max-w-6xl h-full md:h-[85vh] rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row z-10">
              ...
              <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto bg-white flex flex-col">
                <div className="flex justify-between items-start mb-8"><p className="text-[10px] font-black uppercase tracking-widest text-amber-600">Proposed Changes</p><button onClick={() => setSelectedSuggestion(null)} className="text-stone-300 hover:text-stone-900 transition"><X size={28} /></button></div>
                ...
                <div className="flex-1"><p className="text-stone-600 text-lg font-medium leading-relaxed whitespace-pre-wrap">{selectedSuggestion.suggested_description}</p></div>
                <div className="mt-12 pt-10 border-t border-stone-100 flex gap-4">
                  <button onClick={() => handleApproveSuggestion(selectedSuggestion)} className="flex-1 flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] py-5 rounded-2xl bg-green-600 text-white hover:bg-green-700 transition-all shadow-lg"><CheckCircle2 size={18} /> Apply Changes</button>
                  <button onClick={() => handleRejectSuggestion(selectedSuggestion.id)} className="flex-1 flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] py-5 rounded-2xl bg-red-50 text-red-600 hover:bg-red-100 transition-all"><Trash2 size={18} /> Reject</button>
                </div>
              </div>
            </motion.div>
          </div>
